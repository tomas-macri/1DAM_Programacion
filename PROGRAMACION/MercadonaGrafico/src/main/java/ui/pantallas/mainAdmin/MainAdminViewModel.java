...
public class MainAdminViewModel {
    ...
    public void eliminarProducto(Producto producto) {
        MainAdminState mas;
        if (serviciosProductos.eliminarProducto(producto.getNombre())){
            mas = new MainAdminState(null, serviciosProductos.getLista(), null);
        }
        else{
            mas = new MainAdminState(null, null, "No se ha podido eliminar el producto");
        }
        state.setValue(mas);
    }
    ...
    public void eliminarUsuario(Usuario usuario) {
        MainAdminState mas;
        if (serviciosUsuarios.eliminarUsuario(usuario.getDni()) != null){
            List<Usuario> usuarioList = serviciosUsuarios.getLista();
            mas = new MainAdminState(usuarioList, null, null);
        }
        else{
            mas = new MainAdminState(null, null, "No se ha podido eliminar el usuario");
        }
        state.setValue(mas);
    }
}
